...
class  GaussianNB:
	"""GaussianNB就是先验为高斯分布的朴素贝叶斯
		Bayes Theorem:
										        Likelihood(可能性) * Class prior probability（类别先验概率）
				Posterior Probability（后验概率） = -------------------------------------
											    Predictor prior probability（预测先验概率）
	...
	def _calc_class_prior(self):
		...
		for outcome in np.unique(self.y_train):
			...
			self.class_priors[outcome] = outcome_count / self.train_size # 就是每一类的占比

	def _calc_likelihoods(self):
		""" P(x|c) - Likelihood """
		for feature in self.features:
			for outcome in np.unique(self.y_train):
                # 拿到该类别下对应feature的均值和方差
	...
	def predict(self, X):
		...
		for query in X:
			...
			"""
			for outcome in np.unique(self.y_train):
				prior = self.class_priors[outcome] # p(c)
				likelihood = 1
				evidence_temp = 1

				for feat, feat_val in zip(self.features, query):
					mean = self.likelihoods[feat][outcome]['mean']
					var = self.likelihoods[feat][outcome]['variance'] # 方差
                    # 每一类特征的概率的连乘，该特征在c类的概率就是高斯分布的概率密度函数的取值
                    # 朴素贝叶斯假设独立，所以联合概率为连乘
					likelihood *= (1/math.sqrt(2*math.pi*var)) * np.exp(-(feat_val - mean)**2 / (2*var)) # p(x|c) = p(x1|c)*p(x2|c)*...

				posterior_numerator = (likelihood * prior) # p(x|c)*p(c)
				probs_outcome[outcome] = posterior_numerator # 因为P(x)是固定值，相当于是分母，大家都一样
